...
class ConstantBlockTest {
			...
			for(unsigned int t = 0; t < nofTests; t++) {
				...
				for(unsigned int i = 0; i < N; i++) {
					if(!Utils::compareApprox(ref[i], res[i], 0.001)) {
						error++;
						std::cout << "test " << t << ", element " << i << ": Failure (" << value[i] << " -> " << res[i] << " but should be " << ref[i] << ")" << std::endl;
					}
					else {
						std::cout << "test " << t << ", element " << i << ": OK (" << value[i] << " -> " << res[i] << ")" << std::endl;
					}
				}
			}
			return error;
	...
	protected:
		...
		static constexpr unsigned int randomBufferLength = nofTests * N;
...
int main(int argc, char* argv[]) {
	...
	ConstantBlockTest<Matrix<3, 1, int>,    int,    3 * 1> m3x1iTester;
	ConstantBlockTest<Matrix<1, 2, long>,   long,   1 * 2> m1x2lTester;
	ConstantBlockTest<Matrix<2, 2, float>,  float,  2 * 2> m2x2fTester;
	ConstantBlockTest<Matrix<3, 2, double>, double, 3 * 2> m3x2dTester;
	
	ConstantBlockTest<std::array<int,    7>, int,    7> a7iTester;
	ConstantBlockTest<std::array<long,   3>, long,   3> a3lTester;
	ConstantBlockTest<std::array<float,  5>, float,  5> a5fTester;
	ConstantBlockTest<std::array<double, 4>, double, 4> a4dTester;
