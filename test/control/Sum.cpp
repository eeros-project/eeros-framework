...
TEST(controlSumTest, double) {
  Sum<3, double> s1;
  Constant<> c1(2.5), c2(-3.5), c3(10.1);
  s1.getIn(0).connect(c1.getOut());
  s1.getIn(1).connect(c2.getOut());
  s1.getIn(2).connect(c3.getOut());
  c1.run(); c2.run(); c3.run(); s1.run();	
  EXPECT_TRUE(Utils::compareApprox(s1.getOut().getSignal().getValue(), 9.1, 0.001));  
  s1.negateInput(0);
  s1.run();
  EXPECT_TRUE(Utils::compareApprox(s1.getOut().getSignal().getValue(), 4.1, 0.001));  
  s1.negateInput(1);
  s1.run();
  EXPECT_TRUE(Utils::compareApprox(s1.getOut().getSignal().getValue(), 11.1, 0.001));  
  s1.negateInput(2);
  s1.run();
  EXPECT_TRUE(Utils::compareApprox(s1.getOut().getSignal().getValue(), -9.1, 0.001));  
}
