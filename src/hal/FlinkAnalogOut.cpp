...
FlinkAnalogOut::FlinkAnalogOut(std::string id, FlinkDevice* device, uint8_t subDeviceNumber, uint32_t channel, double scale, double offset) : ScalableOutput<double>(id, scale, offset), channel(channel), bitMask(0) {
