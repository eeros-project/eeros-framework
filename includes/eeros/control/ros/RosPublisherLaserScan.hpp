...
class RosPublisherLaserScan : public RosPublisher<sensor_msgs::msg::LaserScan::Type, double> {
 ...
 public:
  RosPublisherLaserScan(const rclcpp::Node::SharedPtr node, const std::string& topic, const std::string& frame_id, const uint32_t queueSize=1000)
      : RosPublisher<TRosMsg, double>(node, topic, queueSize), frame_id(frame_id) { }
  ...
  void setRosMsg(TRosMsg& msg) {
    ...
    if (rangesInput.isConnected() ) {
      ...
      std::vector<float> rangesTmpFloat( rangesTmpDouble.begin(), rangesTmpDouble.end() );
    ...
    if (intensitiesInput.isConnected() ) {
      ...
      std::vector<float> intensitiesTmpFloat( intensitiesTmpDouble.begin(), intensitiesTmpDouble.end() );
