...
/** \brief Safety system.
 *
 * Every robot control system needs exactly one safety system. The safety system
 * comprises several safety levels and safety events to switch between these
 * levels
 */
class SafetySystem : public Runnable {
 public:
   ...
   * @param period The period determines the speed, which the executor will run
   * the safety system.
   */
  ...
  /**
   * Causes a fire a safety event. Make sure that this event is registered for
   * the actual safety level and that it is a public event in case of being
   * fired from outside of the safety system.
   * @param event The safety event to be fired.
   * @param context The context, in which the event is fired, this could be
   * public or private.
   */
  ...
  /**
   * The executor will call this method when the safety system has to run. Do
   * not call manually.
   */
  void run();

  /**
   * This method can be used to shut the system down gracefully when stopping a
   * program with signals, e.g. SIGINT. Register a signal handler and call this
   * function from there.
   */
  static void exitHandler();

  logger::Logger log; /**< This logger is used to put out information about the
                         safety system */
 ...
 private:
  ...
  std::atomic<SafetyLevel*> entryLevel;
